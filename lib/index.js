 ...
 * re-deriving it.
 */
function buildReceipt(label, startedAt, total, { truncated, truncatedFrom, tier, statuses, freshness, ignoredFreshness }) {
	const seconds = ((Date.now() - startedAt) / 1000).toFixed(1);
	const results =
		truncatedFrom !== undefined && truncatedFrom > total
			? `${total} of ${truncatedFrom} results (numResults cap)`
...
class MultiBackendSearchProvider {
	...
	async search(request, signal) {
		...
		// G5 context budget: cap the caller's request before any backend
		// runs, so vendors are never asked for results we would drop.
	...
	/**
	 * Apply the G5 context budget (clamp the result list to `numResults`,
	 * BEFORE any verification traffic is spent on results that would be
	 * dropped), then apply the configured verification tier, tag each kept
	 * source's snippet, and prepend the provenance receipt to `content`
	 * (C-lite). A clamp is announced in the receipt (`N of M results
	 * (numResults cap)`) and in the structured `truncated` flag.
	...
	async #finalize(raw, options, { label, startedAt, signal }) {
		const clampedFrom =
			raw.sources.length > options.numResults ? raw.sources.length : undefined;
		const sources =
			clampedFrom !== undefined ? raw.sources.slice(0, options.numResults) : raw.sources;
		...
		return {
			...raw,
			sources: marked,
			truncated: raw.truncated === true || clampedFrom !== undefined,
