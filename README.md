...
- **`web_search` toolview card on the Web**: the `web_search` row in a conversation takes over the host's built-in web card and adds the provenance receipt line, tone-coded per-source verification badges (`alive`, `verified`, `dead 404`, …), and the truncation notice; when the web seam is not pinned to this provider it degrades gracefully (no receipt line claimed, no badges invented) instead of mislabeling someone else's results
