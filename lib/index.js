...
export async function probeBackends(ctx, options, { timeoutMs = 15_000 } = {}) {
	...
	const run = async (name, label, call) => {
		const startedAt = Date.now();
		try {
			await call(AbortSignal.timeout(timeoutMs));
	...
	const probes = [];
	const exaKey = await resolveKey(ctx, options, "exa");
	probes.push(
		run("exa", exaKey !== undefined ? "exa-rest" : "exa-mcp", (signal) =>
			exaKey !== undefined
				? exaRestSearch(options, exaKey, request, signal)
				: exaMcpSearch(options, request, signal)
		)
	);
	const firecrawlKey = await resolveKey(ctx, options, "firecrawl");
	if (firecrawlKey !== undefined || options.firecrawlKeyless === true) {
		probes.push(run("firecrawl", "firecrawl", (signal) => firecrawlSearch(options, firecrawlKey, request, signal)));
	} else {
		probes.push(Promise.resolve({ name: "firecrawl", label: "firecrawl", status: "disabled", detail: "disabled", ms: 0 }));
	}
	const backends = await Promise.all(probes);
					...
					handler: (req, res) => {
						...
						inFlight.then(
							...
							(error) => {
								...
								// failure is classified into a detail code);
								// this is the defensive path for the unexpected.
								// "Failure never silent": answer 500 with the
								// reason instead of hanging or swallowing.
								res.writeHead(500, { "content-type": "application/json; charset=utf-8" });
								res.end(JSON.stringify({ error: `probe failed: ${String(error?.message ?? error)}` }));
							}
						);
					}
				}),
