 ...
 * and, when the composition has not already done so, also registers the
 * stock model-facing `web_fetch` tool (reusing `@deepseek-ai/dsh-tool-web`'s
 * `applyWebFetchTool`) so the model can call it out of the box. It installs a
 * Settings section (`web-search-ext`) so the Web panel
...
// Static (top-level) import on purpose: dsh-tool-web is a declared peer
// dependency AND an in-box harness package (always resolvable from the
// profile fallback node_modules or the installation). If it is missing, the
// plugin must fail LOUD at load time — a lazy import with a catch would
// silently degrade to the 0.3.0 state (provider installed, tool absent,
// model gets `unknown tool "web_fetch"` with no diagnostic).
import {
	applyWebFetchTool,
...
/**
 * Services this plugin injects: `web` (provider registration seam) plus
 * `tools` + `systemPrompt`, which `applyWebFetchTool` needs to register the
 * stock model-facing `web_fetch` tool (see `registerWebFetchToolIfAbsent`).
 */
const inject = ["web", "tools", "systemPrompt"];
...
const USER_AGENT = "dsh-web-search-ext/0.3.1";
...
const Config = z.object({
	...
	/**
	 * Search result budget (G5): the result count requested when the request
	 * carries no `maxResults`, and a hard cap on results returned — a larger
	 * `maxResults` is clamped to it and the receipt says so. Together with
	 * `maxSnippetChars` this bounds one search's context to
	 * `numResults × maxSnippetChars` of snippet text.
	 */
	...
	/** L1: timeout budget per content-check phase (request and body read each get one). */
...
// ── G3 connectivity probe ──────────────────────────────────────────────────

/**
 * Classify a probe failure into a closed, locale-neutral detail code (G3).
 * The probe payload must stay secret-free (no vendor messages, URLs, or
 * keys — it rides the same route family as the health counters, outside
 * the /api trust fence), so outcomes map to a fixed code set that the
 * client translates via the `probe.*` locale keys:
 *
 *   WEB_RATE_LIMIT        → "rate-limited"
 *   abort (probe's own
 *   per-backend timeout)  → "timeout"
 *   wrapped fetch failure → "network"     (wrapNetworkError's
 *                                         `…request failed:` marker —
 *   checked BEFORE any vendor status text, so a proxy failure whose
 *   message happens to carry an HTTP code still classifies as
 *   transport, not auth)
 *   HTTP 401/402/403      → "auth"        (the backend fns embed the
 *                                         status in their WebError
 *                                         message: `…(HTTP 40x)`)
 *   any other rejection   → "error"       (any other WebError, or a
 *   (non-WebError)        non-WebError rejection such as a
 *                                        credentials-layer failure
 *                                        surfaced through key resolution
 *                                        — local, not transport)
 */
function classifyProbeError(error) {
	if (error instanceof WebError && error.code === "WEB_RATE_LIMIT") return "rate-limited";
	if (error instanceof WebError && error.code === "WEB_ABORTED") return "timeout";
	if (isAbortError(error)) return "timeout";
	const message = error instanceof WebError ? String(error.message ?? "") : "";
	if (message.includes("request failed:")) return "network";
	if (/\(HTTP 40[123]\)/u.test(message)) return "auth";
	return "error";
}

/**
 * Probe every configured backend with a minimal one-result request and
 * report each one's own outcome (G3, first-install diagnostics).
 *
 * Reuses the SAME backend functions a real search runs — exa serves as
 * keyed REST or anonymous hosted MCP depending on `resolveKey`, firecrawl
 * as keyed or keyless — so the probe exercises the exact path the tool
 * would take. The probe request overrides `maxResults` to 1. No failover:
 * a probe reports per backend, it never substitutes one for another.
 *
 * Probes are diagnostics, not searches: they never touch cooldown state
 * and are never recorded in the health counters; the result is stored on
 * the shared health state via `storeProbe` instead.
 *
 * Each probe is bounded by its own `AbortSignal.timeout(timeoutMs)` so one
 * hung backend cannot stall the others. `probeBackends` never rejects —
 * every per-backend failure is classified into a detail code — so the
 * probe route can always answer 200 with a complete, closed payload:
 * `{ at, backends: [{ name, label, status, detail, ms }] }` where
 * status ∈ ok | error | disabled and detail ∈ { ok, rate-limited, auth,
 * timeout, error, network, disabled }.
 *
 * @param {object} ctx - plugin context (key resolution).
 * @param {object} options - current Config.
 * @param {object} [params]
 * @param {number} [params.timeoutMs=15000] - per-backend probe budget.
 * @returns {Promise<{at: number, backends: Array}>}
 */
export async function probeBackends(ctx, options, { timeoutMs = 15_000 } = {}) {
	const request = { query: "connectivity probe", maxResults: 1 };
	// Each variant receives the probe's signal and returns the serving
	// variant for it — `{ label, call }` — or null when the backend is not
	// enabled. Key resolution runs INSIDE the per-backend try: a
	// credentials-layer rejection (a fresh install is where that layer is
	// most likely broken) must classify like any other per-backend failure
	// instead of rejecting the whole probe; the row's label falls back to
	// the plan name when the serving variant is unknown.
	const run = async (name, variant) => {
		const startedAt = Date.now();
		let label = name;
		try {
			const v = await variant(AbortSignal.timeout(timeoutMs));
			if (v === null) return { name, label: name, status: "disabled", detail: "disabled", ms: 0 };
			label = v.label;
			await v.call();
			return { name, label, status: "ok", detail: "ok", ms: Date.now() - startedAt };
		} catch (error) {
			// `label` keeps the serving variant when it was identified
			// before the call failed; it stays the plan name when the
			// failure happened during key resolution.
			return { name, label, status: "error", detail: classifyProbeError(error), ms: Date.now() - startedAt };
		}
	};
	const backends = await Promise.all([
		run("exa", async (signal) => {
			const key = await resolveKey(ctx, options, "exa");
			return key !== undefined
				? { label: "exa-rest", call: () => exaRestSearch(options, key, request, signal) }
				: { label: "exa-mcp", call: () => exaMcpSearch(options, request, signal) };
		}),
		run("firecrawl", async (signal) => {
			const key = await resolveKey(ctx, options, "firecrawl");
			if (key === undefined && options.firecrawlKeyless !== true) return null;
			return { label: "firecrawl", call: () => firecrawlSearch(options, key, request, signal) };
		})
	]);
	return { at: Date.now(), backends };
}
 ...
 * re-deriving it.
 */
function buildReceipt(label, startedAt, total, { truncated, capped, truncatedFrom, tier, statuses, freshness, ignoredFreshness }) {
	const seconds = ((Date.now() - startedAt) / 1000).toFixed(1);
	const capSuffix = capped === true ? " (numResults cap)" : "";
	const results =
		capped === true && truncatedFrom !== undefined && truncatedFrom > total
			? `${total} of ${truncatedFrom} results${capSuffix}`
			: `${total} result${total === 1 ? "" : "s"}${capSuffix}`;
	let line = `web-search-ext: ${label} · ${seconds}s · ${results}`;
...
class MultiBackendSearchProvider {
	...
	#cooldowns = new Map();
	/** Session-scoped health telemetry (C2); null when constructed without a shared state (tests). */
	#health = null;
	 ...
	 * @param {object} [health] - shared session telemetry state (C2).
	 */
	constructor(ctx, resolveOptions, health = null) {
		this.#ctx = ctx;
	...
	/** Raw cooldown entries for the health snapshot: `{ name, at, ms }[]`. */
	cooldownEntries() {
		return [...this.#cooldowns.entries()].map(([name, entry]) => ({
			name,
			at: entry.at,
			ms: entry.ms
		}));
	 ...
	 * `WEB_PROVIDER_ERROR` with one line per failed backend. The result
	 * count is bounded by the `numResults` context budget (G5): a caller
	 * `maxResults` above it is clamped before any backend is asked, and
	 * `#finalize` announces any over-delivery it drops in the receipt.
	 */
	async search(request, signal) {
		throwIfAborted(signal);
		const options = this.#resolveOptions();
		const startedAt = Date.now();
		if (this.#health !== null) noteCall(this.#health, "search");
		// G5 context budget: numResults is a hard cap on results returned, no
		// matter what the caller asks for. A larger maxResults is clamped
		// before any backend runs, so vendors are never asked for results we
		// would drop. The clamp is surfaced in the receipt and the structured
		// `truncated` flag by #finalize; a caller's SMALLER ask is never
		// raised (over-delivery beyond it is dropped quietly — the caller's
		// at-most contract was honored).
		const requestClamped =
			request.maxResults !== undefined && request.maxResults > options.numResults;
		const limitedRequest = requestClamped
			? { ...request, maxResults: options.numResults }
			: request;
		const cap =
			request.maxResults !== undefined
				? Math.min(request.maxResults, options.numResults)
				: options.numResults;
		const plan = [];
		...
		plan.push({
			...
			run: () => (exaKey !== undefined ? exaRestSearch(options, exaKey, limitedRequest, signal) : exaMcpSearch(options, limitedRequest, signal))
		});
		const firecrawlKey = await resolveKey(this.#ctx, options, "firecrawl");
		if (firecrawlKey !== undefined || options.firecrawlKeyless === true) {
			plan.push({ name: "firecrawl", label: "firecrawl", run: () => firecrawlSearch(options, firecrawlKey, limitedRequest, signal) });
		}
		...
		for (const backend of plan) {
			...
			// One record per attempt: if the backend call succeeded, a later
			// post-processing failure must not double-count it as failed.
			let okRecorded = false;
			try {
				const raw = await backend.run();
				if (this.#health !== null) {
					recordBackend(this.#health, "search", backend.name, backend.label, true, attemptAt);
					okRecorded = true;
				}
				return await this.#finalize(raw, options, { label: backend.label, startedAt, signal, cap, requestClamped });
			} catch (error) {
				if (signal?.aborted === true || isAbortError(error)) throw searchAborted(signal?.reason ?? error);
				if (!okRecorded && this.#health !== null) recordBackend(this.#health, "search", backend.name, backend.label, false, attemptAt);
	...
	/**
	 * Apply the G5 context budget (never return more than `min(ask,
	 * numResults)`; BEFORE any verification traffic is spent on results
	 * that would be dropped), then apply the configured verification tier,
	 * tag each kept source's snippet, and prepend the provenance receipt to
	 * `content` (C-lite). The cap notice fires exactly when the caller's
	 * ask exceeded `numResults` (receipt: `N [of M] results (numResults
	 * cap)` + structured `truncated` flag); drops caused by the caller's
	 * own smaller ask stay quiet — its at-most contract was honored.
	 * Verification is purely local HTTP (no vendor quota, no LLM tokens);
	 * `verifyLevel: off` passes results through with only the receipt. An
	 * abort during verification propagates as `WEB_ABORTED` exactly like an
	 * abort mid-search.
	 */
	async #finalize(raw, options, { label, startedAt, signal, cap, requestClamped }) {
		const clampedFrom =
			raw.sources.length > cap ? raw.sources.length : undefined;
		const sources =
			clampedFrom !== undefined ? raw.sources.slice(0, cap) : raw.sources;
		if (this.#health !== null) noteResults(this.#health, sources.length);
		...
		const receipt = buildReceipt(label, startedAt, sources.length, {
			...
			capped: requestClamped === true,
			truncatedFrom: requestClamped === true ? clampedFrom : undefined,
			tier,
			statuses,
		...
		return {
			...raw,
			sources: marked,
			truncated: raw.truncated === true || requestClamped === true,
	...
	async fetch(request, signal) {
		...
		if (firecrawlKey !== undefined || options.firecrawlKeyless === true) {
			plan.push({
				name: "firecrawl",
				label: "firecrawl",
				run: () => firecrawlScrape(options, firecrawlKey, url, maxChars, signal)
			});
		}
		plan.push({ name: "exa-mcp", label: "exa-mcp", run: () => exaMcpFetch(options, url, maxChars, signal) });

		if (this.#health !== null) noteCall(this.#health, "fetch");
		const now = Date.now();
		const failures = [];
		for (const backend of plan) {
			const entry = this.#cooldowns.get(backend.name);
			...
			const attemptAt = Date.now();
			// One record per attempt (see the search loop).
			let okRecorded = false;
			try {
				const result = await backend.run();
				if (this.#health !== null) {
					recordBackend(this.#health, "fetch", backend.name, backend.label, true, attemptAt);
					okRecorded = true;
				}
				return result;
			} catch (error) {
				if (signal?.aborted === true || isAbortError(error)) throw searchAborted(signal?.reason ?? error);
				if (!okRecorded && this.#health !== null) recordBackend(this.#health, "fetch", backend.name, backend.label, false, attemptAt);
				const rateLimited = error instanceof WebError && error.code === "WEB_RATE_LIMIT";
				if (rateLimited) {
					const sec =
...
/**
 * Register the stock `web_fetch` tool if it is not already visible in the
 * current scope.
 *
 * The model-facing `web_fetch` tool is normally registered by the
 * `@deepseek-ai/dsh-tool-web` plugin via the agent-preset layer. However,
 * every shipped preset ships `fetch: false` on its `tool-web` row, and the
 * `dsh web` profile additionally disables the profile-layer `tool-web` row,
 * so no composition path registers `web_fetch` by default. This plugin fills
 * that gap by calling dsh-tool-web's own `applyWebFetchTool` when the tool
 * is absent. The tool's execution routes through `ctx.web.fetch` — which is
 * pinned to this provider (with SSRF-guarded, keyless-capable backends) —
 * so no unguarded local fetch path is introduced.
 *
 * If a preset or profile layer has already registered `web_fetch` in the
 * same scope, this is a no-op to avoid duplicate-registration errors.
 */
function registerWebFetchToolIfAbsent(ctx) {
...
/** Exact path the settings card's Health tab fetches (same origin). */
const HEALTH_ROUTE_PATH = "/web-search-ext/health";

/**
 * Serve the session telemetry as `GET /web-search-ext/health` (C2).
 *
 * Classic plugins on this host cannot add `/api` RPC methods or host
 * events; the extensible host→browser surface is the web server's route
 * table (`dsh-host-webserver`, the `webServer` service — the same
 * mechanism `dsh-client-connection` uses for `/api`). The route is
 * registered only when that service is mounted (web compositions);
 * CLI/headless profiles simply have no route and the card's fetch shows
 * its explicit unavailable line.
 *
 * The payload (lib/health.js buildHealthJson) is counters only — no
 * credentials, no URLs, no query text. The route sits OUTSIDE the `/api`
 * browser-trust fence (that fence is owned by the `/api` handler itself),
 * so the secret-free invariant is load-bearing: do not add sensitive
 * fields to the snapshot.
 */
function registerHealthRoute(ctx, health, searchProvider, fetchProvider) {
	ctx.inject(["webServer"], (sctx) => {
		sctx.effect(
				...
				sctx.webServer.register({
					...
					handler: (req, res) => {
						...
						const body = JSON.stringify(
							buildHealthJson(health, {
								searchCooldowns: searchProvider.cooldownEntries(),
								fetchCooldowns: fetchProvider.cooldownEntries()
							})
						);
						res.writeHead(200, {
			...
			"web-search-ext: /web-search-ext/health route"
		);
	});
}

/** Exact path the settings card's "Test now" button POSTs to (G3). */
const PROBE_ROUTE_PATH = "/web-search-ext/probe";

/**
 * Run a live connectivity probe (G3) as `POST /web-search-ext/probe`.
 *
 * On demand only: the host never probes at apply time, so a fresh install,
 * a host restart, and the e2e-host CI job make zero vendor calls until a
 * browser asks for it. The card's Health tab fires this probe once, when
 * it first opens with no stored result — that is the "first install"
 * moment a user actually sees the card — and its "Test now" button at
 * will.
 *
 * POST runs `probeBackends()` with the CURRENT options (settings edits
 * apply immediately, no restart), stores the result on the shared health
 * state, and answers with the full health wire JSON so the client's
 * existing parse path updates counters and probe in one round trip.
 * Concurrent POSTs coalesce onto a single in-flight probe. No staleness
 * throttle: a "Test now" click must re-test; each probe is two minimal
 * requests bounded by their own 15 s timeout, and keyless vendors answer
 * 429s as data (detail `rate-limited`), not as failures.
 *
 * Secret-free invariant: the response is `buildHealthJson` — counters plus
 * closed probe codes. No vendor messages, no URLs, no keys.
 */
function registerProbeRoute(ctx, health, resolveOptions, searchProvider, fetchProvider) {
	let inFlight = null;
	ctx.inject(["webServer"], (sctx) => {
		sctx.effect(
			() =>
				sctx.webServer.register({
					...
					handler: (req, res) => {
						...
						req.resume(); // the probe takes no input; drain any body
						if (inFlight === null) {
							inFlight = (async () => {
								const result = await probeBackends(ctx, resolveOptions());
						...
						inFlight.then(
							...
							(error) => {
								// probeBackends never rejects (every per-backend
								// failure — key resolution included — is
								// classified into a detail code); this is the
								// defensive path for the unexpected. "Failure
								// never silent": answer 500, but with a CLOSED
								// body — echoing the raw reason here would break
								// the secret-free invariant of this route
								// family. The client surfaces the HTTP status on
								// its explicit failure line.
								res.writeHead(500, {
									"content-type": "application/json; charset=utf-8",
									"cache-control": "no-store"
								});
			...
			"web-search-ext: /web-search-ext/probe route"
		);
	});
}
...
/**
 * Register both providers with `ctx.web`, install the Settings section,
 * ensure the model-facing `web_fetch` tool is available, and serve the
 * session telemetry at GET /web-search-ext/health (C2). `settings.yaml`
 * (or the settings card) edits the same config both providers serve —
...
function apply(ctx, config) {
	...
	// One session-scoped telemetry state shared by both providers.
	const health = createHealthState();
	const searchProvider = new MultiBackendSearchProvider(ctx, () => current(), health);
	const fetchProvider = new MultiBackendFetchProvider(ctx, () => current(), health);
	ctx.web.registerSearchProvider(searchProvider);
	ctx.web.registerFetchProvider(fetchProvider);
	registerWebFetchToolIfAbsent(ctx);
	registerHealthRoute(ctx, health, searchProvider, fetchProvider);
	registerProbeRoute(ctx, health, () => current(), searchProvider, fetchProvider);
}
