...
- **Client entry no longer fails to apply on live hosts**: the `web_search` toolview registration collided with the host's built-in `web-toolview` entry — same keyed slot, same key, same default priority 0 — which the slot registry hard-rejects, so the whole client entry failed to load ("Failed to load plugins") and the settings card, Health tab, and `/search-engine` command all died with it. The registration now shadows the host entry at an explicit priority (the registry renders the lowest-priority entry per key), and the committed bundle's registration is asserted in `test/toolview.test.mjs` because no CI path runs the browser-side registry.
