...
- Test suite grew from 10 to 37 mocked scenarios: L0/L1 markers, receipt shape, HEAD-405/501 fallback, verify-off zero-traffic, 429 retry-after (header + body + string values + clamp) + cooldown, freshness on the wire, fetch failover (429 and non-429), 404-as-result, SSRF guard matrix (IPv6/127-8/FQDN spellings), redirect re-validation, L0 status mapping, L1 deadline + snippet-less `[unverified]`, abort during verification, dual-429, truncation, keyless-only plans.
