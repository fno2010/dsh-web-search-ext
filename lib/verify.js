...
export function isSafeUrl(url) {
	...
	// Normalize: lowercase, strip trailing dots (FQDN form "localhost.", and
	// the empty-label spellings "localhost.." / "127.0.0.1..").
 ...
 */
/** Map a raw probe-error string to a liveness status (see checkLiveness). */
function statusFromError(error) {
	if (error === "timeout") return "timeout";
	if (error === "aborted") return "aborted";
	if (error === "unsafe_url") return "unsafe";
	return "error";
}
...
export async function checkLiveness(urls, { timeoutMs = 3000, signal } = {}) {
	...
	const jobs = urls.map(async (url) => {
		...
		if (outcome.error !== undefined) {
			results.set(url, { status: statusFromError(outcome.error), detail: outcome.error });
			return;
		}
		...
		if (outcome.status === 405 || outcome.status === 501) {
			...
			if (retry.error !== undefined) {
				results.set(url, { status: statusFromError(retry.error), detail: retry.error });
				return;
			}
 ...
 * Timing: the redirect/request phase gets a `timeoutMs` budget (inside
 * manualRedirects) and the body read gets a second identical budget, so a
 * single URL can never consume more than 2 × timeoutMs total. The body
 * deadline is enforced even while a `read()` is in flight: a live timer
 * aborts the request (rejecting the pending read) at the deadline, and the
 * caller's signal is attached to the same controller so a caller abort also
 * interrupts a stalled download.
...
async function fetchPageText(url, { bytes, timeoutMs, signal } = {}) {
	...
	// Keep a live timer armed for the whole body phase: when it fires it
	// aborts the request, which rejects any pending read() or text().
	const bodyTimer = setTimeout(() => outcome.controller?.abort(), Math.max(0, deadline - Date.now()));
	const onAbort = () => outcome.controller?.abort();
	signal?.addEventListener("abort", onAbort, { once: true });
	try {
		...
		let bytesRead = 0;
		while (true) {
			// Early exit between reads; the armed bodyTimer above also
			// covers a read that never settles.
			...
			if (bytesRead > bytes) {
				// We just crossed the byte cap. Retain the part of this chunk
				// that still fits under the cap instead of dropping the whole
				// chunk: dropping it left `length` (capped at `bytes`) larger
				// than the text actually retained, and the caller's
				// min-content-bytes gate could pass on bytes we never kept.
				// (The cap is applied in decoded characters, matching the
				// final slice below — consistent with the pre-existing model.)
				text += decoder.decode(value, { stream: true }).slice(0, Math.max(0, bytes - text.length));
				reader.cancel().catch(() => {});
				break;
			}
