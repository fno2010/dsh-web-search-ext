	...
	async search(request, signal) {
		...
		for (const backend of plan) {
			...
			try {
				...
				if (this.#health !== null) recordBackend(this.#health, "search", backend.name, backend.label, true, attemptAt);
				return await this.#finalize(raw, options, { label: backend.label, startedAt, signal, cap, requestClamped });
			} catch (error) {
				if (signal?.aborted === true || isAbortError(error)) throw searchAborted(signal?.reason ?? error);
				if (this.#health !== null) recordBackend(this.#health, "search", backend.name, backend.label, false, attemptAt);
	...
	async fetch(request, signal) {
		...
		for (const backend of plan) {
			...
			try {
				...
				if (this.#health !== null) recordBackend(this.#health, "fetch", backend.name, backend.label, true, attemptAt);
				return result;
			} catch (error) {
				if (signal?.aborted === true || isAbortError(error)) throw searchAborted(signal?.reason ?? error);
				if (this.#health !== null) recordBackend(this.#health, "fetch", backend.name, backend.label, false, attemptAt);
				const rateLimited = error instanceof WebError && error.code === "WEB_RATE_LIMIT";
				if (rateLimited) {
					const sec =
