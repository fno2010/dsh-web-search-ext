...
function extractRetryAfterSeconds(body) {
	...
	const match = text.match(/retry[_\s-]?after["']?\s*[:=]\s*(\d{1,9})/iu);
 ...
 */
function rateLimitError(label, body, options) {
	const reported = extractRetryAfterSeconds(body);
...
async function exaRestSearch(options, apiKey, request, signal) {
	...
	if (response.status === 429) {
		throw rateLimitError("Exa REST", await safeBody(response), options);
	}
...
async function exaMcpSearch(options, request, signal) {
	...
	if (response.status === 429) {
		throw rateLimitError("Exa MCP", await safeBody(response), options);
	}
	if (!response.ok) {
...
async function firecrawlSearch(options, apiKey, request, signal) {
	...
	if (response.status === 429) {
		throw rateLimitError("Firecrawl", await safeBody(response), options);
	}
	if (response.status === 401 || response.status === 402 || response.status === 403) {
		throw new WebError(
			apiKey === undefined
...
async function firecrawlScrape(options, apiKey, url, maxChars, signal) {
	...
	return {
		url: finalUrl,
		statusCode,
		body: { kind: markdown !== undefined ? "text" : "html", content: truncateTo(text, maxChars) },
		truncated
	};
}
